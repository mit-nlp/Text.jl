...
@expect sparse_count(lines[1], bkg)   == convert(SparseMatrixCSC, sparsevec(Dict{Int64, Float64}(bkg["a"] => 1.0, bkg["b"] => 1.0, bkg["c"] => 1.0), vocab_size(bkg)))
@expect sparse_count(lines[end], bkg) == convert(SparseMatrixCSC, sparsevec(Dict{Int64, Float64}( bkg[unk_token] => 1.0 ), vocab_size(bkg)))

@info "sparse[c] = $(sparse_count(lines[1], bkg)[2])"
@expect Text.norm(sparse_count(lines[1], bkg), bkg)[2] == 3.166666666666667
